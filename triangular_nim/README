...
本環境使用一個 int 代表遊戲狀態，使用 15 個 bit 分別表示 15 個圓圈是否被消除。

使用 0 代表未消除，1 代表已消除。

依照上圖所示，

state 值為 1，代表 1 已消除，其他未消除。

state 值為 2，代表 2 已消除，其他未消除。

state 值為 3，代表 1 和 2 已消除，其他未消除。

state 值為 4，代表 3 已消除，其他未消除。

以此類推。

Environment 類別中提供了幾個轉換方法方法：
